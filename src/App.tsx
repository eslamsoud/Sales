...
import { Product, Customer, Invoice, Expense, FactoryLoad, AppSettings, Trip, UserAuth, SyncLog } from './types';
...
export default function App() {
  ...
  const promptForSync = (actionDesc: string) => {
    ...
    setTimeout(async () => {
      if (window.confirm(`✅ تم التسجيل محلياً (${actionDesc}).\n\nلحماية بياناتك من الضياع في حالة مسح الكاش للمتصفح، يُنصح بمزامنة وتأكيد العملية في جوجل شيت الآن.\nهل توافق على الرفع للسحابة؟`)) {
        const success = await syncAllDataToGoogle(true);
        if (success) {
          alert('☁️ تم حفظ ومزامنة البيانات في جوجل شيت السحابي بنجاح!');
        } else {
          alert('❌ حدث خطأ أثناء المزامنة، تأكد من اتصالك بالإنترنت وصحة رابط جوجل شيت.');
        }
      }
    }, 400);
  };
  ...
  const handleAddInvoice = (newInvoice: Omit<Invoice, 'id'>) => {
    ...
    // Keep the inline background notification just in case they cancel the full sync, but we remove the inline POST since we prompt full sync.
