...
export default function FactoryTab({
  ...
  const activeWeights = useMemo(() => {
    ...
    return getProductWeightsFallback(activeProductObj).filter(w => w.cartonPriceFromFactory > 0);
  }, [activeProductObj]);
                  ...
                  <div className="border border-indigo-100 rounded-xl p-3.5 bg-indigo-50/30 flex flex-col gap-3">
                    ...
                    <div className="bg-[#FFFFFF] p-3 rounded-lg border border-slate-150 grid grid-cols-2 gap-2.5">
                      ...
                      <button
                        ...
                        className={`col-span-2 \${editingWeightId ? 'bg-[#DD6B20]' : 'bg-indigo-50 text-[#1A365D]'} hover:bg-opacity-80 text-white font-bold py-1.5 px-3 rounded-md text-xs border border-indigo-200 active:scale-95 transition-all text-center flex items-center justify-center gap-1 cursor-pointer mt-1`}
                      >
                        <Plus className="h-4 w-4" />
