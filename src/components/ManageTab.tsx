...
import { Product, AppSettings, Customer, Invoice, Expense, Trip, FactoryLoad, getProductWeightsFallback, UserAuth, formatNum, SyncLog } from '../types';
import { 
...
export default function ManageTab({
  ...
  const checkGeminiStatus = async () => {
    try {
      const response = await fetch('https://sales-six-pied.vercel.app/api/gemini/status');
      if (response.ok) {
  ...
  const handleMarketSearch = async (queryText: string) => {
    ...
    try {
      const response = await fetch('https://sales-six-pied.vercel.app/api/gemini/market-research', {
        method: 'POST',
...
معك مندوب شركة "الأخوة المتحدون" لزيوت وسمن سوفانا الفاخرة ذو الرائحة الفريدة.
...
بصفتي خبير المبيعات الاحتياطي لشركة الأخوة المتحدون، وبناءً على فئة العميل **[ ${aiChatCategory} ]** والسياسة الإرشادية لمدير المبيعات: *"${pitchGuidelines}"*، إليك التكتيك الأمثل:
    ...
    try {
      const payload = {
        ...
        customers: customers.map(c => ({
          ...
          name: c.name,
          phone: c.phone,
          detailedAddress: (c as any).detailedAddress || '',
          locationLink: c.locationLink || '',
          nationalId: (c as any).nationalId || ''
        })),
        factoryLoads: factoryLoads.map(l => {
          const prod = products.find(p => p.id === l.productId);
          const w = prod?.weights?.find(wt => wt.id === l.weightId);
          return {
            date: l.date,
            productName: prod ? prod.name : 'مجهول',
            weightSize: w ? w.size : 'غير محدد',
            quantity: l.quantity,
            cartonsCount: l.cartonsCount || 0,
            advanceAmount: l.advanceAmount || 0,
            warehouseKeeper: l.warehouseKeeper || ''
          };
        })
      };
          ...
          <div className="flex flex-col gap-4 animate-fade-in text-right">
            ...
            <div className="flex flex-wrap bg-slate-100 p-1 rounded-2xl gap-1 border border-slate-200">
              ...
              <button
                ...
                className={`flex-1 min-w-[120px] py-1.5 px-1 rounded-xl text-xs font-black transition-all cursor-pointer select-none text-center ${
                  managerSubTab === 'sync_logs' ? 'bg-[#FFFFFF] text-[#1A365D] shadow-sm font-extrabold' : 'text-slate-500 bg-transparent'
                }`}
              >
                ☁️ سجل المزامنة
              </button>
                ...
                <div className="bg-[#FFFFFF] p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col gap-4">
                  ...
                  {usersList.length === 0 ? (
                    <div className="bg-slate-50 rounded-2xl p-4 text-center border border-slate-150 text-xs font-bold text-slate-400">
                      لا يوجد مستخدمون مسجلون بعد. قم بإضافة المندوبين من النموذج أعلاه ليتمكنوا من الدخول للنظام.
                    </div>
                  ) : (
  ...
  try {
    ...
    if (data.type === 'تقرير_كامل') {
      ...
      if (!customersSheet) {
        ...
        customersSheet.appendRow(['المحافظة', 'المنطقة', 'اسم العميل', 'رقم الهاتف', 'العنوان', 'رابط جوجل ماب', 'الرقم القومي']);
      ...
      if (data.customers && data.customers.length > 0) {
        ...
        var currRows = data.customers.map(function(c) {
          return [c.governorate || '', c.area || '', c.name || '', c.phone || '', c.detailedAddress || '', c.locationLink || '', c.nationalId || ''];
        });
      ...
      if (!factorySheet) {
        factorySheet = ss.insertSheet('المصنع');
        factorySheet.appendRow(['التاريخ', 'اسم الصنف', 'الحجم/الوزن', 'الكمية (كرتونة)', 'إجمالي الوحدات', 'مقدم المصنع', 'أمين المخزن']);
        factorySheet.getRange(1, 1, 1, factorySheet.getLastColumn()).setFontWeight("bold").setBackground("#fce5cd");
      }
      if (data.factoryLoads && data.factoryLoads.length > 0) {
        if (factorySheet.getLastRow() > 1) {
          factorySheet.getRange(2, 1, factorySheet.getLastRow() - 1, factorySheet.getLastColumn()).clearContent();
        }
        var factoryRows = data.factoryLoads.map(function(fl) {
          return [fl.date, fl.productName, fl.weightSize, fl.cartonsCount, fl.quantity, fl.advanceAmount, fl.warehouseKeeper];
        });
        factorySheet.getRange(2, 1, factoryRows.length, factoryRows[0].length).setValues(factoryRows);
      }
                            ...
                            return (
                              <div key={weight.id} className="border border-slate-150 p-3 rounded-xl bg-[#F7FAFC]/30 flex flex-col gap-2">
                                ...
                                <div className="grid grid-cols-3 gap-2 text-right">
                                  ...
                                  {weight.cartonPriceFromFactory > 0 ? (
                                    <div>
