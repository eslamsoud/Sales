...
export default function PricesTab({ products: rawProducts, onGoBack }: PricesTabProps) {
  const [activeSubTab, setActiveSubTab] = useState<'prices' | 'calc' | 'whatsapp_bot'>('prices');
  ...
  const buildAllPricesText = () => {
    ...
    productPriceDetails.forEach(prod => {
      ...
      prod.weights.forEach(w => {
        text += `🔹 _سعة ${w.size}_:\n`;
        text += `  • تجزئة: قطعه: ${formatPriceWithCurrencyAndDecimal(w.retailPricePerUnit)} | كرتونة: ${formatPriceWithCurrencyAndDecimal(w.cartonPriceFromFactory)}\n`;
  ...
  const calcWeightsList = useMemo(() => {
    ...
    return getProductWeightsFallback(selectedCalcProduct).filter(w => w.cartonPriceFromFactory > 0);
  }, [selectedCalcProduct]);
