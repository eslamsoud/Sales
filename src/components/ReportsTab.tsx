...
import { Invoice, Expense, Product, Customer, Trip, AppSettings, formatNum, FactoryLoad, getProductWeightsFallback, UserAuth } from '../types';
...
export default function ReportsTab({
  ...
  const [activeSubTab, setActiveSubTab] = useState<'finance' | 'stats' | 'areas' | 'invoices' | 'inventory' | 'activity'>(() => {
    if (permittedSubTabs && permittedSubTabs.length > 0) {
      ...
      if (permittedSubTabs.includes('reports_activity')) return 'activity';
    }
    return 'finance';
  });
  ...
  const exportMonthlyReportAsPDF = (monthStr: string, displayDate: string, sales: number, revenuesParam: number, expensesParam: number, profit: number) => {
    const dataUrl = exportMonthlyReportAsPNG(monthStr, displayDate, sales, revenuesParam, expensesParam, profit, true);
    if (!dataUrl) return;
    ...
    const cHeight = 350 + totalLines * 35 + 150;
    
    const pdfWidth = 210;
    const pdfHeight = (cHeight * pdfWidth) / 650;
    ...
    doc.addImage(dataUrl as string, 'PNG', 0, 0, pdfWidth, pdfHeight, undefined, 'FAST');
    doc.save(`تقرير_شهر_${monthStr}.pdf`);
  };
...
أريد فقط نص الرسالة بدون أي مقدمات أخرى لتكون جاهزة للإرسال.`;
  ...
  const exportMonthlyReportAsPNG = (monthStr: string, displayDate: string, sales: number, revenuesParam: number, expensesParam: number, profit: number, returnDataUrl = false) => {
    ...
    ctx.beginPath();
    ctx.arc(canvas.width - 60, 70, 30, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = '#ffffff';
    ctx.textAlign = 'center';
    ctx.font = 'bold 13px system-ui, sans-serif';
    ctx.fillText('الأخوة', canvas.width - 60, 75);

    ctx.fillStyle = '#ffffff';
    ctx.fillRect(0, 0, canvas.width, canvas.height);

    ctx.fillStyle = '#312e81';
    ctx.fillRect(15, 20, canvas.width - 30, 115);

    ctx.fillStyle = '#ffffff';
    ctx.textAlign = 'center';
    ctx.font = 'bold 24px system-ui, sans-serif';
    ctx.fillText('تقرير الحسابات الختامية - ' + displayDate, canvas.width / 2, 55);

    ctx.font = '500 13px system-ui, sans-serif';
    ctx.fillStyle = '#c7d2fe';
    ctx.fillText('تم التصدير من نظام المستودع والمبيعات', canvas.width / 2, 80);

    const repName = settings.representativeName || 'المندوب الأساسي';
    ctx.fillStyle = '#fde047';
    ctx.font = 'bold 13px system-ui, sans-serif';
    ctx.fillText(`إعداد / المندوب: ${repName}`, canvas.width / 2, 105);
    ...
    <div className="bg-[#F7FAFC] min-h-screen pb-12 font-sans" id="reports-tab-container">
      ...
      <div className="max-w-xl mx-auto p-4 flex flex-col gap-4">
        ...
        {(() => {
          ...
          const showActivity = !permittedSubTabs || permittedSubTabs.length === 0 || permittedSubTabs.includes('reports_activity');
          return (
            <div className="flex flex-wrap bg-[#FFFFFF] p-2 rounded-2xl border border-slate-200 gap-1 sm:gap-2 shadow-sm text-center">
              ...
              {showActivity && (
                <button
                  ...
                  className={`flex-1 py-1.5 px-1 rounded-xl font-black text-[11px] sm:text-[13px] transition-all cursor-pointer select-none whitespace-nowrap ${
                    activeSubTab === 'activity' ? 'bg-[#FFFFFF] text-[#1A365D] border-b-2 border-b-[#DD6B20] shadow-sm rounded-none' : 'text-[#9CA3AF] bg-transparent border-transparent'
                  }`}
                >
                  نشاط المناديب
                </button>
        ...
        {activeSubTab === "inventory" && (
          <div className="flex flex-col gap-4 animate-fade-in text-right" dir="rtl">
            <div className="bg-[#FFFFFF] p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col gap-3">
              <h3 className="font-bold text-[#1A365D] text-sm border-b border-slate-100 pb-2 flex items-center gap-1.5">
                <Check className="h-4.5 w-4.5 text-[#2B6CB0]" />
                مراجعة ومطابقة المخزون مع المندوب
              </h3>
              <p className="text-xs text-slate-500 leading-relaxed mt-1 mb-2">
                يوضح هذا التقرير مقارنة دقيقة بين إجمالي المسحوبات (المُحملة من أمانة المصنع للسيارة) وإجمالي المبيعات للعملاء، لمعرفة المتبقي الفعلي (الجرد) وكشف أي عجز، أو زيادة، أو تلاعب غير مبرر في رصيد المندوب.
              </p>
              
              <div className="flex flex-col gap-3 max-h-[60vh] overflow-y-auto pr-1">
                {products.map(product => {
                  const items = getProductWeightsFallback(product);
                  return items.map(weight => {
                    // Total loaded
                    const totalLoadedUnits = factoryLoads
                      ?.filter(l => l.productId === product.id && l.weightId === weight.id)
                      .reduce((sum, l) => sum + (l.quantity || 0), 0) || 0;
                    
                    // Total sold
                    const totalSoldUnits = invoices
                      .flatMap(inv => inv.items)
                      .filter(item => item.productId === product.id && item.weightId === weight.id)
                      .reduce((sum, item) => sum + (item.quantity || 0), 0) || 0;
                    
                    const remainingUnits = totalLoadedUnits - totalSoldUnits;
                    const stockAlertThreshold = product.minStockAlert || 10;
                    
                    if (totalLoadedUnits === 0 && totalSoldUnits === 0) return null; // Skip if no activity

                    const unitsPerCarton = weight.unitsPerCarton || 12;
                    const labelUnit = product.accountingUnit || 'كرتونة';
                    const remainingCartons = Math.floor(remainingUnits / unitsPerCarton);
                    const remainingPieces = remainingUnits % unitsPerCarton;

                    return (
                      <div key={`${product.id}-${weight.id}`} className="p-3 bg-slate-50 border border-slate-200 rounded-xl flex flex-col gap-2 transition-all hover:shadow-xs">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200 pb-2 mb-1">
                          <div className="flex items-center gap-2">
                            <span className="font-extrabold text-[#1A365D] text-sm">
                              {product.name}
                            </span>
                            <span className="bg-sky-100 text-sky-950 px-2.5 py-0.5 rounded-lg text-[10px] font-black border border-sky-200 shadow-sm">
                              {weight.size}
                            </span>
                          </div>
                          
                          <div className={`px-2 py-1.5 rounded-lg text-[11px] font-black border flex items-center justify-center gap-1.5 shadow-sm min-w-[130px] ${
                            remainingUnits < 0 
                              ? 'bg-rose-100 text-rose-800 border-rose-300' 
                              : remainingUnits <= stockAlertThreshold
                                ? 'bg-amber-100 text-amber-800 border-amber-300'
                                : 'bg-emerald-50 text-emerald-800 border-emerald-200'
                          }`}>
                            <span>الرصيد المتبقي:</span>
                            <span dir="ltr">
                              {remainingCartons} {labelUnit} {remainingPieces > 0 ? ` + ${remainingPieces} ق` : ''} 
                            </span>
                          </div>
                        </div>
                        
                        <div className="grid grid-cols-2 gap-3 text-center">
                          <div className="bg-indigo-50/50 border border-indigo-100 rounded-xl p-2.5 flex flex-col gap-1.5">
                            <span className="text-[10px] font-bold text-indigo-900 border-b border-indigo-100/50 pb-1.5">إجمالي المنصرف من المصنع</span>
                            <span className="font-black text-indigo-700 font-mono text-base">{formatNum(totalLoadedUnits)} <span className="text-[9px] font-bold font-sans">عبوة</span></span>
                          </div>
                          <div className={`bg-teal-50/50 border border-teal-100 rounded-xl p-2.5 flex flex-col gap-1.5 ${totalSoldUnits > 0 ? 'opacity-100' : 'opacity-60 grayscale'}`}>
                            <span className="text-[10px] font-bold text-teal-900 border-b border-teal-100/50 pb-1.5">إجمالي المبيعات (المسجل)</span>
                            <span className="font-black text-teal-700 font-mono text-base">{formatNum(totalSoldUnits)} <span className="text-[9px] font-bold font-sans">عبوة</span></span>
                          </div>
                        </div>

                        {remainingUnits < 0 && (
                          <div className="mt-1 flex items-center justify-center gap-1.5 p-1.5 bg-rose-50 text-rose-700 rounded-lg border border-rose-100 text-[10px] font-black">
                            <ShieldAlert className="h-3.5 w-3.5 shrink-0" />
                            <span>عجز: المبيعات المسجلة تتجاوز الرصيد المحُمل للمندوب من المصنع!</span>
                          </div>
                        )}
                      </div>
                    );
          ...
          <div className="flex flex-col gap-4 animate-fade-in text-right" dir="rtl">
            <div className="bg-[#FFFFFF] p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col gap-3">
              <h3 className="font-bold text-[#1A365D] text-sm border-b border-slate-100 pb-2 flex items-center gap-1.5">
                ...
                مراقبة نشاط الميدان (آخر ظهور للمناديب)
              </h3>
              <p className="text-xs text-slate-500 leading-relaxed mt-1 mb-2">
                يوضح هذا التقرير حالة الاتصال والمزامنة الخاصة بكل مندوب لمراقبة العمليات بشكل لحظي.
              </p>

              <div className="flex flex-col gap-3">
                {usersList.length === 0 ? (
                  <div className="text-center py-6 text-slate-500 text-xs font-bold bg-slate-50 rounded-xl border border-slate-100">
                    لا يوجد مناديب مسجلين حالياً
                  </div>
                ) : (
                  usersList.map((user, idx) => {
                    const isOnline = user.lastActive && (new Date().getTime() - new Date(user.lastActive).getTime()) < 1000 * 60 * 15; // 15 mins
                    return (
                      <div key={idx} className="p-3 bg-slate-50 border border-slate-200 rounded-xl flex flex-col gap-2 transition-all hover:shadow-xs">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200 pb-2 mb-1">
                          <div className="flex items-center gap-2">
                           <div className="relative">
                             <div className="bg-indigo-100 h-8 w-8 rounded-full flex items-center justify-center text-indigo-700 font-black text-sm border border-indigo-200">
                               {user.name.charAt(0)}
                             </div>
                             <span className={`absolute bottom-0 right-0 h-2.5 w-2.5 rounded-full border border-white ${isOnline ? 'bg-emerald-500' : 'bg-slate-300'}`}></span>
                           </div>
                            <div className="flex flex-col">
                              <span className="font-extrabold text-[#1A365D] text-sm">
                                {user.name}
                              </span>
                              <span className="text-[10px] text-slate-500 font-bold">{user.phone} - {user.customRoleName || 'مندوب'}</span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3 text-center">
                          <div className="bg-indigo-50/50 border border-indigo-100 rounded-xl p-2.5 flex flex-col gap-1.5">
                            <span className="text-[10px] font-bold text-indigo-900 border-b border-indigo-100/50 pb-1.5 flex items-center justify-center gap-1"><Clock className="h-3 w-3" /> آخر ظهور</span>
                            <span className="font-black text-indigo-700 text-xs mt-1" dir="ltr">
                              {user.lastActive ? new Date(user.lastActive).toLocaleString('ar-EG', { dateStyle: 'short', timeStyle: 'short' }) : 'غير متوفر'}
                            </span>
                          </div>
                          <div className={`bg-teal-50/50 border border-teal-100 rounded-xl p-2.5 flex flex-col gap-1.5`}>
                            <span className="text-[10px] font-bold text-teal-900 border-b border-teal-100/50 pb-1.5 flex items-center justify-center gap-1"><Send className="h-3 w-3" /> آخر مزامنة</span>
                            <span className="font-black text-teal-700 text-xs mt-1" dir="ltr">
                              {user.lastSync ? new Date(user.lastSync).toLocaleString('ar-EG', { dateStyle: 'short', timeStyle: 'short' }) : 'لم يقم بالمزامنة'}
                            </span>
                          </div>
                        </div>
                      </div>
