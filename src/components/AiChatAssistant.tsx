...
import { MessageCircle, X, Send, Loader2, Maximize2, Minimize2, Mic, Volume2, Square, Sparkles, Database, RotateCcw, ThumbsUp, ThumbsDown } from 'lucide-react';
import Markdown from 'react-markdown';
import { Product, Customer, Invoice, Expense, FactoryLoad, Trip, UserAuth, AppSettings } from '../types';
...
export default function AiChatAssistant({
  ...
  // دالة لتوليد رسالة الترحيب ديناميكياً بناءً على الوقت (رسالة تذكيرية صباحية)
  const getInitialGreeting = (botName: string) => {
    const hour = new Date().getHours();
    const isMorning = hour >= 5 && hour <= 12;
    if (isMorning) {
      return `صباح الطموح والنجاح يا بطل المبيعات! ☀️\nمعك صديقك، خبيرك الاستراتيجي ورفيق دربك "${botName}". لقد قمت للتو بمراجعة جرد سيارتك وحسابات عملائك.\n\nهل أنت مستعد لسماع "خطة العمل المقترحة لليوم" لننطلق بقوة وحماس ونكتسح السوق؟ 🚀`;
    }
    return `مرحباً بك يا بطل المبيعات مع صديقك، خبيرك الاستراتيجي ورفيق دربك "${botName}"! 👋\n\nأنا مطلع على جرد السيارة وعملائك. يمكنك سؤالي عن الفئات المستهدفة لأصنافك، ترتيب خطة زياراتك، أو حتى إخباري بنتيجة زيارتك الأخيرة لنحللها معاً. كيف نكتسح السوق الآن يا صديقي؟ 🚀`;
  };

  const [isMinimized, setIsMinimized] = useState(false);
  const [messages, setMessages] = useState<Array<{ id?: string, role: 'user' | 'model', text: string, rating?: 'up' | 'down' }>>([
    {
      id: Date.now().toString(),
      role: 'model',
      text: getInitialGreeting(aiBotName)
    }
  ]);
  ...
  const getContextString = (): string => {
    try {
      ...
      // 3. Active customers by Area (for geographic routing)
      const customersByArea: Record<string, string[]> = {};
      customers.forEach(c => {
        const area = c.area || 'غير محدد';
        if (!customersByArea[area]) customersByArea[area] = [];
        customersByArea[area].push(c.name);
      });
...
=== التوزيع الجغرافي للعملاء (لخطط السير) ===
${areaList || '- لا يوجد عملاء مسجلين.'}
...
`;
  ...
  const playAudio = (text: string, index: number) => {
    ...
    // تطبيق الصوت المختار من الإعدادات إن وجد
  ...
  const handleSendQuery = async (queryText: string) => {
    ...
    if (cleanMsg === 'جديد' || cleanMsg === 'جديد ' || cleanMsg === 'new' || cleanMsg === 'reset' || cleanMsg === 'أريد خطة جديدة' || cleanMsg === 'اعادة تهيئة') {
      ...
      setMessages([
        {
          role: 'model',
          text: `تمت إعادة تهيئة صديقك "${aiBotName}" مجدداً بنجاح! 🔄\n\nالبطاقات التفاعلية والتخطيط الميداني جاهز.\n\n${getInitialGreeting(aiBotName)}`
        }
      ]);
      return;
    }
    ...
    try {
      ...
      const systemPrompt = `أنت الصديق المقرب، والمستشار النفسي، والخبير الاستراتيجي الميداني الأول لمندوبي ومسؤولي المبيعات لشركة EAGS للأغذية المتحدون.
اسمك هو "${aiBotName}".
تتحدث برزانة ووعي سيكولوجي وتخطيطي عميق باللغة العربية كخبير مبيعات وتخطيط استراتيجي للأعمال.
مهمتك الكبرى هي:
1. الاستراتيجية الميدانية: توجيه المندوب حول أنواع البضائع المناسبة، أهم الفئات المستهدفة للتعامل معها، كيفية إيجادهم، وطرق الدخول إليهم.
2. تخطيط خط السير: اقتراح ترتيب الزيارات (من يزور أولاً لفتح السيولة أو تحقيق تارجت سريع).
3. المتابعة والتقييم: حين يخبرك المندوب بـ "نتيجة زيارة قام بها"، قم بتحليل مدى نجاحها، وما هي التكتيكات والأفعال التي يجب عليه القيام بها في الزيارة القادمة لهذا العميل لتطوير العلاقة وزيادة السحب.
4. خطة السير الجغرافية: بناءً على التوزيع الجغرافي المتاح لك، اقترح أسماء عملاء محددين لزيارتهم في منطقة واحدة لتقليل المسافات وتوفير البنزين.
4. دعم المندوب كأخ وصديق، وتحليل شخصية العميل نفسياً لتقديم تكتيكات إقناع لإبرام الصفقات وتسوية الديون.

أوامر صارمة للاستجابة:
- يمنع منعاً باتاً استخدام الردود المعلبة أو القوالب الجاهزة المكررة. يجب أن يكون ردك متفاعلاً بنسبة 100% مع السياق الراهن (جرد السيارة الحالي الـ Live، والديون).
- التجاوب يجب أن يتم بمنتهى الدقة والمصداقية بناءً على الأرقام المعطاة لك.
- كن حماسياً جداً، ملهماً، وتبث طاقة إيجابية جبارة ومعدية في نفس المندوب لتشجيعه على التكسير وتحقيق التارجت.
- قم بتفكيك نفسية العميل وطريقة تفكيره.
- قدم خططاً استراتيجية وتكتيكات بيعية واضحة (خطوات 1، 2، 3).
- صغ له سيناريو حواري مقنع وجذاب بالعامية المصرية ومقنع للأطراف.
- اطرح عليه دائماً في نهاية ردك سؤالاً استباقياً للمتابعة (مثال: طمني، ناوي تبدأ بمين في خط السير؟ أو إيه كانت نتيجة زيارتك للتاجر ده؟).
...
تحدث معه من منطلق صديق وخبير بيع وموجه سيكولوجي رائع.`;
  ...
  const handleRateMessage = (index: number, rating: 'up' | 'down') => {
    ...
    // Save evaluation to local storage to measure AI success rates
    const logs = JSON.parse(localStorage.getItem('ai_feedback_logs_sys') || '[]');
    logs.push({
      date: new Date().toISOString(),
      message: newMessages[index].text,
      rating: rating,
      delegate: currentUser?.name || 'مجهول'
    });
    localStorage.setItem('ai_feedback_logs_sys', JSON.stringify(logs));
    showToast(rating === 'up' ? 'شكراً لتقييمك الإيجابي! تم الحفظ للتطوير. 👍' : 'شكراً لتقييمك، سنتعلم من هذا لتطوير الذكاء الاصطناعي. 👎');
  };
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[9px] text-indigo-200 leading-tight">صديقك، موجهك التفاوضي ومحلل مبيعاتك</p>
            </div>
          </div>
                  ...
                  <div className={`max-w-[90%] p-3 rounded-2xl text-xs leading-relaxed ${
                    ...
                    {m.role === 'model' ? (
                      <div className="flex flex-col gap-2">
                        ...
                        <div className="flex justify-between items-center pt-2 border-t border-slate-100 mt-1 gap-2">
                          <div className="flex items-center gap-1.5">
                            <span className="text-[8px] text-slate-400 font-sans tracking-tight ml-2">EAG AI</span>
                            {/* Feedback System */}
                            <button 
                              onClick={() => handleRateMessage(idx, 'up')}
                              className={`p-1 rounded-md transition-colors ${m.rating === 'up' ? 'text-emerald-600 bg-emerald-50' : 'text-slate-300 hover:text-emerald-500 hover:bg-emerald-50'}`}
                              title="تقييم إيجابي (خطة ناجحة)"
                            >
                              <ThumbsUp className="h-3 w-3" />
                            </button>
                            <button 
                              onClick={() => handleRateMessage(idx, 'down')}
                              className={`p-1 rounded-md transition-colors ${m.rating === 'down' ? 'text-rose-600 bg-rose-50' : 'text-slate-300 hover:text-rose-500 hover:bg-rose-50'}`}
                              title="تقييم سلبي (خطة غير مفيدة)"
                            >
                  ...
                  <div className="grid grid-cols-1 gap-2 mt-1">
                    <button
                      ...
                      onClick={() => triggerQuickAction(
                        "اقتراح خطة زيارات جغرافية للمسافات المتقاربة",
                        "اقترح عليّ أسماء عملاء محددين لزيارتهم اليوم بناءً على تواجدهم في نفس النطاق الجغرافي أو المحافظة. الهدف هو توفير الوقت والبنزين، مع توضيح أي الأصناف تناسبهم من الجرد الحالي."
                      )}
                    ...
                    >
                      <span className="text-xs font-black text-sky-700">📍 خطة سير مجمعة جغرافياً (توفير بنزين)</span>
                      <span className="text-[9px] text-slate-500 font-sans">تحديد عملاء في نفس المنطقة لزيارتهم معاً</span>
                    </button>

                    <button
                      type="button"
                      onClick={() => triggerQuickAction(
                        "خطة عمل اليوم المقترحة",
                        "صباح الخير! أريد منك تحليل جرد السيارة الراهن والمديونيات بدقة، ووضع 'خطة عمل مقترحة لليوم' تتضمن: 1. الأصناف التي يجب أن أركز على بيعها اليوم بناءً على الأرصدة المتوفرة. 2. ترتيب مقترح للفئات والعملاء لزيارتهم. 3. جرعة حماسية جبارة للبدء بقوة واكتساح السوق وتحقيق التارجت."
                      )}
                      className="p-2 bg-emerald-50 border border-emerald-200 hover:border-emerald-400 rounded-xl text-right transition-all cursor-pointer flex flex-col gap-0.5 justify-center hover:bg-emerald-100/50 shadow-sm"
                    >
                      <span className="text-xs font-black text-emerald-950">☀️ خطة عمل الصباح المقترحة</span>
                      <span className="text-[9px] text-emerald-700 font-bold">تحليل جرد الصباح وترتيب زيارات اليوم للنجاح</span>
                    </button>
                    ...
                    <button
                      type="button"
                      onClick={() => triggerQuickAction(
                        "الاستراتيجية وتحديد الفئات المستهدفة",
                        "بناءً على البضائع الموجودة معي في جرد السيارة الآن، من هي أهم الفئات المستهدفة التي يجب أن أركز عليها اليوم؟ وكيف أرتب خط سير زياراتي (من أزور أولاً ولماذا)؟"
                      )}
                      className="p-2 bg-slate-50 border border-slate-200 hover:border-indigo-400 rounded-xl text-right transition-all cursor-pointer flex flex-col gap-0.5 justify-center hover:bg-indigo-50/20"
                    >
                      <span className="text-xs font-black text-teal-950">🗺️ استراتيجية خط السير والفئات المستهدفة</span>
                      <span className="text-[9px] text-slate-500 font-sans">تحديد لمن نبيع اليوم وترتيب الزيارات لضمان النجاح</span>
                    </button>

                    <button
                      type="button"
                      onClick={() => {
                        const input = prompt("اكتب ملخصاً لنتيجة زيارتك الأخيرة (مثال: العميل رفض بسبب السعر، أو العميل طلب كمية قليلة):");
                        if (input) {
                          triggerQuickAction(
                            "تقييم زيارة سابقة ومتابعة ميدانية",
                            `لقد قمت للتو بزيارة عميل، وهذه كانت نتيجة الزيارة: "${input}". 
أريد منك تحليل مدى نجاح هذه الزيارة، وما هي الأخطاء المحتملة التي ارتكبتها، وما الذي يجب عليّ فعله بالتحديد في الزيارة القادمة لهذا العميل لرفع كفاءة السحب؟`
                          );
                        }
                      }}
                      className="p-2 bg-slate-50 border border-slate-200 hover:border-indigo-400 rounded-xl text-right transition-all cursor-pointer flex flex-col gap-0.5 justify-center hover:bg-indigo-50/20"
                    >
                      <span className="text-xs font-black text-rose-950">🎯 تقييم زيارة عميل والمتابعة الميدانية</span>
                      <span className="text-[9px] text-slate-500 font-sans">تحليل نتيجة زيارتك الأخيرة وتخطيط التحرك القادم للعميل</span>
                    </button>
