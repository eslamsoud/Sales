...
function getSimulatedAIResponse(message: string): string {
  ...
  // Extract possible customer name (e.g. "ماركت الأمل", "أبو أحمد") if mentioned in customer search/objections
  ...
  // 2. Scenario-based Adaptive Responses
  
  // A. Discount Theme (الخصومات والتسهيلات السعرية والكميات)
  ...
  if (msg.includes("غاضب") || msg.includes("صعب") || msg.includes("معترض") || msg.includes("يرفض") || msg.includes("زعلان") || msg.includes("مشاكل") || msg.includes("مشكله") || msg.includes("شكوى") || msg.includes("اشتك") || msg.includes("شكا")) {
    ...
    return `😤 **بروتوكول التعامل المعرفي والذكاء الوجداني مع الاعتراضات وصعوبات المقابلة**:\n\nمواجهة اعتراضات ${targetName} هي اللحظة الحقيقية التي تظهر مهارة المندوب المحترف. الاعتراض ليس رفضاً للمنتج، بل هو طلب مبطن لمزيد من التطمين المرن. اتبع هذه الخطوات الـ 3 المعتمدة في سوفانا:\n\n1️⃣ **خطوة الامتصاص الهادئ (أذنان وفم واحد)**:\n- دع ${targetName} يفرغ شحنته تماماً ويعبر عن عتبه بخصوص الأسعار أو مواعيد التوصيل. وافقه وجدانياً أولاً لكي تسحب سلاح الغضب منه: *"أنت على حق تماماً يا معلم، وضع السوق صعب ويهمنا جداً مصلحتك ويهمنا راحتك، وأنا هنا خصيصاً اليوم لأحل لك هذا الأمر."*.\n\n2️⃣ **سيناريو "السلعة التجريبية والعينات"**:\n- إذا كان رافضاً لتجربة صنف جديد من سمن أو زيت سوفانا بحجة أن الزبائن لم يعتادوا عليها، اعرض عليه كرتونة واحدة بتسهيل شديد أو مجاناً ليعرضها ويرى بنفسه سرعة دورانها وإيمان الزبون بجودتها الفائقة.\n\n3️⃣ **سيناريو "التعويض الفوري غير المالي"**:\n- إذا اشتكى من عيب في التعبئة أو تأخير لفاتورة سابقة، قم بتقديم تعويض مباشر مرن: كرتونة مجانية هداية تفجيرية، أو وعود مؤكدة على التطبيق بأولوية التوريد الصباحي المباشر لسيارته.\n\n🚀 *تذكر دائماً*: العميل المعترض والصعب هو أفضل عملاء المستقبل إذا استطعت بمرونتك وأسلوبك الراقي كسب ثقته اليوم!`;
  }

  // D. General query / greetings / system functions
  ...
  app.post("/api/gemini/market-research", async (req, res) => {
    try {
      ...
      if (!key || key.trim() === "" || key.includes("YOUR_API_KEY")) {
        ...
        return res.json({
          text: `📊 **مؤشر البورصة الاقتصادي المدمج للزيوت والسلع (تحليل مستقل دون مفتاح API)**\n\nبناءً على تتبع الأسواق لسلعة: "${query}":\n\n- **استقرار العرض**: تظهر المؤشرات الحالية توافر مخزونات جيدة من زيت الصويا وزيت الأولين بالموانئ المصرية، مسجلةً اتجاهاً نحو الثبات النسبي بعد التقلبات الفائتة.\n- **سعر الطن التقريبي لمصانع التكرير**: \n  * زيت الصويا الخام المستورد: يتراوح بين 53,000 إلى 54,000 ج.م للطن صب.\n  * زيت الصويا المكرر المعبأ: يتراوح بين 56,000 إلى 57,500 ج.م للطن.\n  * زيت النخيل (الأولين RBD) درجة أولى: يسجل قرابة 54,500 ج.م للطن.\n\n- **التوصيات التجارية لسيارات التوزيع**: يُنصح بتفعيل الخصومات النقدية الفورية (عبر تطبيق مندوب مبيعات سوفانا) لتسريع تحصيل السيولة النقدية وتدوير رأس المال لزيادة الأرباح الإجمالية للسيارة.🚀`,
          sources: [
            { title: "مؤشر أسواق الزيوت والسلع بموانئ الإسكندرية ودمياط", uri: "#" },
            { title: "بورصة السلع المصرية - قطاع الزيوت النباتية", uri: "#" }
          ]
        });
      }
